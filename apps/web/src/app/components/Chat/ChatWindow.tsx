...
console.log(messages);
  ...
  async function sendUserInsights(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
    ...
    const userChat = JSON.stringify(user);
    console.log(userChat);
    const response = await axios.post("http://localhost:3000/api/userInsights", {userChat});
  ...
  return (
    <div className={`flex flex-col items-center p-4 md:p-8 rounded grow m-20 overflow-hidden `}>
      ...
      {(count < 6)?
        ...
        <form className="flex flex-col" onSubmit={sendUserInsights}>
          <button type="submit" className="shrink-0 px-8 py-4 bg-sky-600 rounded w-30 ">
            Go to Dashboard
          </button>
        </form>
  }
